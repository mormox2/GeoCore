 ...
 */
export function formatChatWidgetResponse(payload) {
    const badgeColor = payload.hallucinationRisk === "low"
        ? "#10b981"
        : payload.hallucinationRisk === "medium"
            ? "#f59e0b"
            : "#f43f5e";
    const sourcesList = payload.sourcesCited
        .map((s) => `<li><a href="${s.url || '#'}" target="_blank" rel="noopener">📄 ${s.title}</a> <span style="font-size:0.75rem; color:#38bdf8;">[Trust: ${s.trustLevel}]</span></li>`)
        .join("\n");
    return `
