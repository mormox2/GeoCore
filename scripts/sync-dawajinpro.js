...
async function syncDawajinPro() {
  console.log(`\n🐔 GeoCore Production Exporter ➔ Dawajin Pro`);
  console.log(`=============================================`);

  const configPath = path.resolve(__dirname, "../examples/dawajinpro/geocore.config.json");
  ...
  if (!report.valid) {
    console.error(`❌ Échec de validation pour Dawajin Pro:`);
  ...
  const outputDir = path.resolve(__dirname, "../dist/dawajinpro");
  ...
  const destDir = process.env.DAWAJINPRO_DEST_DIR;
