...
export async function searchHybrid(
  ...
  for (let i = 0; i < lexicalItems.length; i++) {
    ...
    if (existing) {
      ...
      if (!existing.summary) existing.summary = item.summary;
    } else {
  ...
  for (let i = 0; i < vectorHits.length; i++) {
    ...
    if (existing) {
      ...
      existing.rrfScore += rrfContribution;
      if (!existing.matchedChunkText) existing.matchedChunkText = hit.document.text;
    } else {
      const obj = objMap.get(objectId);
      itemMap.set(objectId, {
        objectId,
        title: (hit.document.metadata?.title as string) || obj?.title || objectId,
        slug: (hit.document.metadata?.slug as string) || obj?.slug,
        language: (hit.document.metadata?.language as string) || obj?.language,
  ...
  const results: HybridSearchResultItem[] = sorted.slice(0, limit).map((item) => {
    let matchType: "both" | "lexical-only" | "semantic-only" = "semantic-only";
