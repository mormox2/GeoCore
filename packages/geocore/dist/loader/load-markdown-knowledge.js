...
import { GC_LOADER_INPUT_CONTENT_MISSING, GC_LOADER_INPUT_CONTENT_INVALID, } from "../validation/validation-codes.js";
/**
 * Loads a KnowledgeObject from raw Markdown knowledge input.
 */
export function loadMarkdownKnowledge(input) {
    const diagnostics = [];
    ...
    const hasParseErrors = fmDiagnostics.some((d) => d.severity === "error" || d.severity === "critical");
    if (hasParseErrors) {
        return { diagnostics };
    }
