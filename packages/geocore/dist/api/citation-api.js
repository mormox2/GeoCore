 ...
 */
export function getSource(dataset, request) {
    ...
    if (visibility === "public") {
        if (source.status !== "active" || source.visibility !== "public") {
            return createForbiddenResponse(`Source '${request.id}' is not publicly available.`);
        }
    }
    else if (visibility === "internal") {
        if (source.visibility === "private") {
            return createForbiddenResponse(`Source '${request.id}' is private and not accessible.`);
        }
    }
 ...
 */
export function listSources(dataset, request = {}) {
    const visibility = request.visibility ?? "public";
 ...
 */
export function getCitation(dataset, request) {
    const visibility = request.visibility ?? "public";
    ...
    if (visibility === "public" && citation.status !== "active") {
        return createForbiddenResponse(`Citation '${request.id}' is not active and not publicly available.`);
    }
 ...
 */
export function listCitations(dataset, request = {}) {
    const visibility = request.visibility ?? "public";
