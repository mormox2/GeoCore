 ...
 */
export function generateImageObjectSchema(asset) {
    const output = {
    ...
    if (asset.canonicalUrl)
        output.url = asset.canonicalUrl;
    if (asset.source)
        output.contentUrl = asset.source;
    if (asset.mimeType)
        output.encodingFormat = asset.mimeType;
    if (asset.width)
        output.width = `${asset.width}px`;
    if (asset.height)
        output.height = `${asset.height}px`;
    if (asset.caption)
        output.caption = asset.caption;
    if (asset.license)
        output.license = asset.license;
    if (asset.credit)
        output.creditText = asset.credit;
    if (asset.language)
        output.inLanguage = asset.language;
    return output;
}
/**
 * Generates a Schema.org VideoObject for a video media asset.
 */
export function generateVideoObjectSchema(asset) {
    const output = {
        "@context": "https://schema.org",
        "@type": "VideoObject",
        name: asset.title,
    ...
    if (asset.thumbnailId)
        output.thumbnailUrl = asset.thumbnailId;
    if (asset.language)
        output.inLanguage = asset.language;
    return output;
}
 ...
 */
export function generateMediaSchema(asset) {
    if (asset.type === "image" ||
        asset.type === "diagram" ||
