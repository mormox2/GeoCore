 ...
 */
function formatUrlElement(entry) {
    const lines = [];
    lines.push("  <url>");
    ...
    for (const alt of entry.alternates) {
        lines.push(`    <xhtml:link rel="alternate" hreflang="${escapeXml(alt.language)}" href="${escapeXml(alt.url)}"/>`);
    }
    // Image extensions
 ...
 */
function buildUrlsetHeader(hasAlternates, hasImages) {
    const namespaces = [
 ...
 */
export function generateSitemapIndexXml(sitemapUrls) {
    const lines = [];
