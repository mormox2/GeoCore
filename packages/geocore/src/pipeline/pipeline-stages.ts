...
export function runDatasetValidationStage(
...
export function runKnowledgeObjectsValidationStage(
  dataset: KnowledgeDataset
): ValidationPipelineStageResult {
  const startedAt = new Date().toISOString();
...
export function runRelationshipsValidationStage(
  dataset: KnowledgeDataset
): ValidationPipelineStageResult {
  const startedAt = new Date().toISOString();
...
export function runSearchValidationStage(input: {
  ...
  const issues: ValidationIssue[] = (searchIndex.diagnostics || []).map((diag, index) => ({
...
export function runSitemapValidationStage(input: {
  ...
  const issues: ValidationIssue[] = (sitemapOutput.diagnostics || []).map((diag, index) => ({
