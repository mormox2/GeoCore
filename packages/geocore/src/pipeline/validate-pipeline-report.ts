...
import { ValidationPipelineReport } from "../types/validation-report.js";
...
export function validatePipelineReport(
  ...
  if (report.stages && Array.isArray(report.stages)) {
    ...
    report.stages.forEach((stage, idx) => {
      ...
      if (stage.status === "passed") cntPassed++;
      else if (stage.status === "warning") cntWarning++;
      else if (stage.status === "failed") cntFailed++;
      else if (stage.status === "skipped") cntSkipped++;
    });
