...
import { SitemapDiagnostic } from "../types/sitemap-diagnostic.js";
import { SitemapEntry } from "../types/sitemap-entry.js";
import { filterSitemapPublicObjects } from "./sitemap-filter.js";
import {
  createSitemapOutputId,
  createSitemapEntryId,
  isSitemapPublicObject,
  resolveSitemapUrl,
  extractSitemapImages,
 ...
 */
export function generateSitemap(input: GenerateSitemapInput): SitemapOutput {
  ...
  for (const obj of publicObjects) {
    ...
    const entry: SitemapEntry = {
      ...
      lastModified: meta?.updatedAt ?? (obj as any).updatedAt ?? undefined,
  ...
  if (!validation.valid || validation.issues.length > 0) {
    for (const issue of validation.issues) {
      const exists = diagnostics.some((d) => d.code === issue.code && d.sourceId === (issue as any).sourceId);
      if (!exists) {
        diagnostics.push({
          id: issue.id,
