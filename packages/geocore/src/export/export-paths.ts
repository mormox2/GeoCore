...
export function createExportPath(input: {
  ...
  if (type === "search-index") return "search-index.json";
  if (type === "llms") return "llms.txt";
  if (type === "llms-full") return "llms-full.txt";
  if (type === "sitemap") return "sitemap.xml";
...
function extensionForType(type: StaticExportAssetType): string {
  switch (type) {
    case "markdown":
      return ".md";
    case "json":
