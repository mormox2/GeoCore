...
export function detectOrphans(registry: GraphRegistry): ValidationIssue[] {
  ...
  for (const rel of registry.relationships) {
    if (rel.sourceId) referencedNodeIds.add(rel.sourceId);
    if (rel.targetId) referencedNodeIds.add(rel.targetId);
  }
