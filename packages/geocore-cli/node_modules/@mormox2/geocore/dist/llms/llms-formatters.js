 ...
 */
export function formatLlmsLink(title, url) {
    if (url) {
        return `[${title}](${url})`;
    }
    return title;
 ...
 */
export function formatLlmsList(items) {
...
export function formatLlmsMetadataBlock(input) {
    ...
    if (input.canonicalUrl)
        parts.push(`Canonical URL: ${input.canonicalUrl}`);
    if (input.trustLevel)
        parts.push(`Trust Level: ${input.trustLevel}`);
    if (input.freshness)
        parts.push(`Freshness: ${input.freshness}`);
    return parts.join("\n");
}
/**
 * Returns plain string content, or formats objects as fenced JSON blocks.
 */
export function stringifyLlmsContent(content) {
