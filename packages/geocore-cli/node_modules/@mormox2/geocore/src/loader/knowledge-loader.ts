...
import { loadRelationship } from "./load-relationship.js";
...
export function loadKnowledgeDataset(input: LoadKnowledgeInput): KnowledgeDataset {
  ...
  for (const raw of input.inputs) {
    ...
    switch (raw.type) {
      ...
      case "relationship": {
        const { relationship, diagnostics: diags } = loadRelationship(raw);
        dataset.diagnostics.push(...diags);
        if (relationship) dataset.relationships.push(relationship);
        break;
      }
      default: {
        dataset.diagnostics.push(
